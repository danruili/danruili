---
outline: deep
layout: doc
---
# Danrui Li

## Education

- Ph.D. Student in Computer Science, Rutgers University, USA, 2022-Present
- M.S. in Architecture, Tongji University, China, 2019-2022
- B.Arch. in Architecture, Chongqing University, China, 2014-2019

## Work Experience

### Research Assistant
Rutgers University–New Brunswick.
Sep 2022 - Present

- Utilized LLM agent systems and graph-based knowledge base to assist the ideation, implementation and evaluation of card games.
- Developed multi‑modal document QA and recommender system for architectural design using mLLMs and knowledge graph.
- Integrated LLMs with Unity to orchestrate generative tools that transform natural‑language instructions into digital stories.
- Developed pedestrian models for high-density environments. Performed crowd safety simulations for mass gatherings in public infrastructure. 


### Urban Design Intern

Shanghai Tongji Urban Planning & Design Institute Co., Ltd.
Dec 2019 - Jan 2020 

- Built benchmark tools for sensitivity analysis in pedestrian simulations using MassMotion SDK.
- Reduced expected maximum pedestrian congestion in a railway station design by simulation-assisted architectural design optimization.
- Proposed and refined architecutural design for railway station areas.

### Consulting Intern

Cushman & Wakefield.
Jul 2019 - Aug 2019

- Investigated project strengths and weakness for clients by geographic data analysis.
- Proposed develop strategies for real estate and economic zones projects in China.

### Urban Design Intern

Zhuheng Design Group.
Sep 2018 - Dec 2018
- Assisted in urban design projects, including site analysis, design concept development, modeling, and presentation preparation.